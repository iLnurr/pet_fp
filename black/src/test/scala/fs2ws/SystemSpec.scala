...
import fs2ws.impl.doobie.{ DoobieService, TableReader, TableWriter, UserReader, UserWriter }
import org.scalatest.flatspec.AnyFlatSpec
import org.scalatest.matchers.should.Matchers

import scala.concurrent.ExecutionContext

class SystemSpec extends AnyFlatSpec with Matchers with BeforeAndAfterAll with StrictLogging {
  ...
  it should "subscribe/unsubscribe client" in {
    testWebsockets(
      msgsToSend = List(login("admin", "admin"), subscribe_tables(), unsubscribe_tables()),
      expected = List(login_successful(UserType.ADMIN), table_list(Seq()), Domain.empty)
    ).unsafeRunSync()
  }
  it should "properly add table" in {
    ...
    testWebsockets(
      msgsToSend = List(login("admin", "admin"), add_table(-1, t), add_table(0, t2)),
      expected = List(
  ...
  it should "properly update table" in {
    ...
    testWebsockets(
      msgsToSend = List(login("admin", "admin"), add_table(-1, t), update_table(updated)),
      expected = List(
        login_successful("admin"),
        table_added(-1, t),
  ...
  it should "properly remove table" in {
    ...
    testWebsockets(
      msgsToSend = List(login("admin", "admin"), add_table(-1, t), remove_table(id)),
      expected = List(
        login_successful("admin"),
        table_added(-1, t),
        table_removed(id)
