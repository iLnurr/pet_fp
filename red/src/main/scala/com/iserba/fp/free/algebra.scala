...
import com.iserba.fp.utils.Free.{~>, freeMonad, Translate}
...
object interpreters {
  ...
  class AlgInterpreter[Alg[_]](
    implicit monad: Monad[({ type f[a] = Free[Alg, a] })#f]
  ) {
...
object compilers {
  implicit val connFreeMonad
    : Monad[({ type f[a] = Free[ConnectionAlg, a] })#f] =
